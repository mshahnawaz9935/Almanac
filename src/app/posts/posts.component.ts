...
export class PostsComponent implements OnInit {
  ...
  constructor(private http:Http,private DataService: DataService, private sanitizer: DomSanitizer, private router:Router) { 
    ...
    this.http.get('http://angular2ap.azurewebsites.net/api/posts?topic='+ this.data.topic + '&chapter='+ this.data.chapter + '&moduleid='+ this.DataService.moduleid + '&modulename=' + this.DataService.modulename + '&articleid=' + this.data.articleid )
  ...
  saveonenote()
  {
      // window.open('http://angular2ap.azurewebsites.net/onenote/writenote','_self');
       alert('Saved to One Note');
        this.http.get('http://angular2ap.azurewebsites.net/onenote/writenote')
        .map((res: Response) => res.json()).subscribe((dataFromServer) => {
  ...
  {
    this.loading = true;
    this.http.get('http://angular2ap.azurewebsites.net/onenote/checknote3')
        .map((res: Response) => res.json()).subscribe((dataFromServer) => {
  ...
  checknote()
  {
       this.http.get('http://angular2ap.azurewebsites.net/onenote/checknote2')
        .map((res: Response) => res.json()).subscribe((dataFromServer) => {
  ...
  { 
          this.saved = true;
          this.loading = true;
         this.http.get('http://angular2ap.azurewebsites.net/api/getdata')
        .map((res: Response) => res.json()).subscribe((dataFromServer) => {
